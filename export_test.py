...
v_tasas = ['Tasa Contagios','Tasa Recup','Tasa muertes']   # Nombre de las variables
P = 5   # Número de registros utilizados para test

# Crear una lista de fechas para las cuales se van a generar los pronósticos
date = pd.to_datetime(cum_Med.loc[len(cum_Med)-1,['Fecha']])  
Fecha = pd.date_range(start=date[0],periods=90,freq='D')
...
for c in ciudades:
  ...
  for i in v_tasas:
    ...
    prueba = np.array(name_df[i])                # Seleccionar columna de cada tasa
    train, test = prueba[:-P], prueba[-P:]       # Crear Train y Test
    ...
    # Predicción del modelo
    pred_aj = model_arima_fit.predict(typ='levels')
    pred = model_arima_fit.forecast(90+P)[0]
    name_df['Pred '+str(i)] = [None]*param[c][1] + pred_aj.tolist() + pred[:P].tolist()

    # Agregar a dataframe de predicciones cada columna (variable)
    globals()['pred_90_tasas'+ str(c)][i] = np.round(model_arima_fit.forecast(90+P)[0][P:])
